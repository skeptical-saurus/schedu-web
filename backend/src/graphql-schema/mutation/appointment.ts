...
export const approveAppointment = AppointmentTC.mongooseResolvers.updateById({ record: { isRequired: false } }).wrapResolve((next) => async (rp) => {
...
export const rejectAppointment = AppointmentTC.mongooseResolvers.updateById({ record: { isRequired: false } }).wrapResolve((next) => (rp) => {
